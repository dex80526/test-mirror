...
#
kubectl get configmaps job-config >/dev/null 2>&1
if [ $? != 0 ]; then
kubectl create configmap job-config --from-file=all-presets.yaml=cluster/jobs/all-presets.yaml \
 ...
 --from-file=istio.release-builder.release-1.5.gen.yaml=cluster/jobs/aspenmesh/release-builder/istio.release-builder.release-1.5.gen.yaml   
